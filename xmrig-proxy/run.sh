...
git clone https://github.com/xmrig/xmrig-proxy.git ; cd xmrig-proxy
...
mkdir build ; cd build ; cmake .. -DCMAKE_C_COMPILER=gcc-11 -DCMAKE_CXX_COMPILER=g++-11 -DCMAKE_BUILD_TYPE=Release ; make && cp xmrig-proxy /usr/bin
...
xmrig-proxy -a ${ALGO} --url ${POOL} --user ${WALLET} --rig-id ${WORKER} --pass ${WORKER} --http-host 0.0.0.0 --http-port 8080
fi
